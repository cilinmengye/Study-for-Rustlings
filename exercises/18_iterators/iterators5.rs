...
fn count_iterator(map: &HashMap<String, Progress>, value: Progress) -> usize {
    ...
    map.values().fold(0, |mut count: usize, val| {
        if *val == value {
            count += 1;
        }
        count
...
fn count_collection_iterator(collection: &[HashMap<String, Progress>], value: Progress) -> usize {
    ...
    //               { "variables2": Complete, … }, … ]
    let cnts: Vec<usize> = collection.iter().map(|hashmap| {
        hashmap.values().fold(0, |mut count: usize, val| {
            if *val == value {
                count += 1;
            }
            count
    ...
    cnts.iter().fold(0, |count: usize, val: &usize| {
        count + *val
    })
}
